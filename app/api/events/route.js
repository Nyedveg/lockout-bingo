...
import { getEventsLog, StoreNotConfiguredError } from "../../../lib/store";
...
export async function GET(request) {
  ...
  try {
    const events = await getEventsLog();
    return NextResponse.json({ events, exportedAt: Date.now() }, { headers: { "Cache-Control": "no-store" } });
  } catch (err) {
