...
export function createInitialState() {
  ...
  return {
    ...
    // Note: `events` (the exportable audit log) deliberately isn't part of
    // this "core" shape — it lives in its own spot in the database (see
    // lib/store.js) so routine polling never has to pay for it. Mutators
    // still call logEvent(), which appends to a transient `events` array
    // that lib/store.js adds to the draft before each mutation and strips
    // out (persisting it separately) afterwards.
  };
}

export function pushLog(state, text) {
  const prev = Array.isArray(state.log) ? state.log : [];
  state.log = [{ ts: Date.now(), text }, ...prev].slice(0, 20);
}
