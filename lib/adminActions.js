...
export function adminSetCell(state, cellId, updates) {
  ...
  if (typeof updates.task === "string" && updates.task !== cell.task) {
    const before = cell.task;
    cell.task = updates.task;
    logEvent(state, "edit_task", { cellId, before, after: updates.task });
  }

  if (typeof updates.reservedFor !== "undefined" && updates.reservedFor !== cell.reservedFor) {
    const before = cell.reservedFor;
    ...
    if (updates.reservedFor) {
      ...
      pushLog(state, `Reservation cleared on "${cell.task}"`);
    }
    logEvent(state, "reserve", { cellId, teamId: updates.reservedFor, before });
  }

  const oldMultiplier = cell.multiplier;
  const newMultiplier = typeof updates.multiplier === "number" ? updates.multiplier : cell.multiplier;
  const multiplierChanged = newMultiplier !== oldMultiplier;
  ...
  if (claimedByChanging) {
    ...
    if (cell.claimedBy) {
      ...
      owner.score += BASE_POINTS * (newMultiplier - oldMultiplier);
    }
    cell.multiplier = newMultiplier;
    pushLog(state, `"${cell.task}" multiplier set to x${newMultiplier}`);
    logEvent(state, "multiplier_change", { cellId, before: oldMultiplier, after: newMultiplier });
  }
...
export function swapCells(state, cellIdA, cellIdB) {
  ...
  const beforeA = a.task;
  const beforeB = b.task;
...
// the "Apply randomly" button, and the engine behind the combo family of
// wheel results.
export function applyRandomMultiplier(state, multiplier) {
  ...
  const before = cell.multiplier;
  cell.multiplier = multiplier;
  pushLog(state, `Square ${cell.id + 1} ("${cell.task}") set to x${multiplier} (random)`);
  logEvent(state, "multiplier_change", { cellId: cell.id, before, after: multiplier, detail: `x${multiplier} (random)` });
  return state;
}

// Loads many task texts at once from "N;text" lines (see /admin's
// batch-load box). `tasks` is a { [0-indexed cellId]: text } map —
// parsing happens client-side so bad lines can be reported individually.
export function bulkSetTasks(state, tasks) {
  let count = 0;
  Object.entries(tasks || {}).forEach(([key, text]) => {
    const id = Number(key);
    const cell = state.board[id];
    if (cell && typeof text === "string" && text.trim()) {
      const before = cell.task;
      const after = text.trim();
      if (before !== after) {
        cell.task = after;
        logEvent(state, "edit_task", { cellId: id, before, after });
        count += 1;
      }
    }
  });
  pushLog(state, `Admin batch-loaded ${count} task${count === 1 ? "" : "s"}`);
  logEvent(state, "bulk_set_tasks", { detail: `${count} tasks` });
  return state;
}
...
export function wheelShuffleBoard(state) {
  ...
  const beforeTasks = openCells.map((c) => c.task);
  ...
  openCells.forEach((cell, i) => {
    ...
    if (cell.task !== before) {
      logEvent(state, "edit_task", { cellId: cell.id, before, after: cell.task });
    }
  });
...
// Unclaims up to `count` random claimed squares — Oops unclaims 1,
// Double Oops unclaims 2.
export function wheelUnclaimRandom(state, count = 1) {
  for (let n = 0; n < count; n++) {
    const claimed = state.board.filter((c) => c.claimedBy);
    if (!claimed.length) {
      pushLog(state, "No claimed squares left to undo");
      return state;
    }
...
// `durationMs` lets weaker/stronger wheel variants (Quick Blindness, Total
// Eclipse) reuse the same curse mechanism with a different length.
export function curseTeam(state, teamId, kind, durationMs) {
  const team = state.teams[teamId];
  if (!team) throw new Error("Invalid team");
  if (!state.curses) state.curses = {};
  if (!state.curses[teamId]) state.curses[teamId] = {};
  if (kind === "blindness") {
    const ms = durationMs || BLINDNESS_MS;
    state.curses[teamId].blindUntil = Date.now() + ms;
    pushLog(state, `${team.name} has been cursed with Blindness for ${Math.round(ms / 60000)} minutes!`);
    logEvent(state, "curse_blindness", { teamId, detail: `${Math.round(ms / 60000)}m` });
  } else if (kind === "timeDilation") {
    const ms = durationMs || TIME_DILATION_MS;
    state.curses[teamId].timeDilationUntil = Date.now() + ms;
    pushLog(state, `${team.name} has been cursed with Time Dilation for ${Math.round(ms / 60000)} minutes!`);
    logEvent(state, "curse_time_dilation", { teamId, detail: `${Math.round(ms / 60000)}m` });
  } else {
...
// Total Eclipse: curses every team at once.
export function curseAllTeams(state, kind, durationMs) {
  Object.keys(state.teams).forEach((id) => curseTeam(state, Number(id), kind, durationMs));
  pushLog(state, "🌑 Total Eclipse! Every team is cursed!");
  return state;
}
...
// `stakes` lets Petty Gamble / High Stakes Gamble reuse the same flow with
// a smaller or bigger wager than the standard GAMBLE_STAKES.
export function startGamble(state, teamA, teamB, stakes) {
...
export function startPrisonersDilemma(state) {
  state.pendingPrompt = {
    ...
    status: "voting",
    votes: {}, // { [clientId]: { teamId, choice: "silence" | "snitch" } }
    createdAt: Date.now(),
  };
...
export function resolvePrisonersDilemma(state) {
  ...
  const tally = {};
  teamIds.forEach((id) => (tally[id] = { silence: 0, snitch: 0 }));
  Object.values(prompt.votes || {}).forEach((v) => {
    if (tally[v.teamId] && (v.choice === "silence" || v.choice === "snitch")) {
      tally[v.teamId][v.choice] += 1;
    }
  });
  // Majority per team; ties default to "snitch" (the risk-averse read).
  const teamChoice = {};
  teamIds.forEach((id) => {
    const t = tally[id];
    teamChoice[id] = t.silence > t.snitch ? "silence" : "snitch";
  });
  ...
  for (let i = 0; i < teamIds.length; i++) {
    for (let j = i + 1; j < teamIds.length; j++) {
      ...
      if (ca === "silence" && cb === "silence") {
        netPoints[a] += PD_PAYOFF.bothSilent;
        netPoints[b] += PD_PAYOFF.bothSilent;
      } else if (ca === "snitch" && cb === "snitch") {
        netPoints[a] += PD_PAYOFF.bothSnitch;
        netPoints[b] += PD_PAYOFF.bothSnitch;
      } else if (ca === "snitch" && cb === "silence") {
        netPoints[a] += PD_PAYOFF.snitchBonus;
        netPoints[b] += PD_PAYOFF.silentPenalty;
      } else {
        netPoints[b] += PD_PAYOFF.snitchBonus;
        netPoints[a] += PD_PAYOFF.silentPenalty;
      }
    }
  }
