...
function isConflictError(err) {
  return (
    ...
    (err.name === "BlobPreconditionFailedError" ||
      /precondition/i.test(String(err && err.message)) ||
...
// first request the app has ever served. If two requests both try to
// create it at the same instant, the loser just re-reads the winner's
...
export async function getState() {
  ...
  try {
    ...
    if (isConflictError(err)) {
      const raceWinner = await readState();
      if (raceWinner) return raceWinner.data;
    }
    throw err;
  }
}
...
// mutate it in place (or return a new object). Both the read and the
// write are retried together, since either can go stale under a race.
export async function updateState(mutator, attempts = 6) {
  let lastErr;
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      // Brief jittered pause so competing requests don't retry in lockstep.
