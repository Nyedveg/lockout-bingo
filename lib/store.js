...
// Public: read the current state, initializing it if this is the very
// first request the app has ever served. That initialization goes
// through the SAME transaction-based updateState() as every other
// write (via a no-op mutator) rather than a plain overwrite — otherwise
// a burst of simultaneous first-ever reads (e.g. right when everyone
// loads the app around the same moment) can race each other, and
// whichever plain write lands last silently resets anything another
// request — including an admin action that already started the clock
// — had just correctly written.
export async function getState() {
  const existing = await readState();
  if (existing) return existing;
  return updateState((draft) => draft);
}
...
//
// Same principle as readState() above: only a GENUINE "nothing at this
// path" (Firebase passes null/undefined into a transaction callback when
// the location is empty) is treated as safe to initialize fresh. Data
// that exists but fails the shape check throws instead of silently
// regenerating — this function is caught by updateState()'s own
// try/catch, which aborts the transaction and re-throws to the caller,
// so nothing gets committed.
function toValidState(currentData) {
  ...
  const cloned = structuredClone(currentData);
  if (!Array.isArray(cloned.board) || cloned.board.length !== BOARD_CELLS) {
    console.error(
      "[store] Transaction saw CORE data that exists but fails shape validation — refusing to silently reset it.",
      JSON.stringify({
        boardType: typeof cloned.board,
        boardIsArray: Array.isArray(cloned.board),
        boardLength: cloned.board && cloned.board.length,
      })
    );
    throw new Error(
      "Saved game data exists but is in an unexpected shape. This needs a look rather than being silently reset."
    );
  }
  const state = normalizeState(cloned);
  state.events = [];
  return state;
}
