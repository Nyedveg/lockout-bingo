...
export function acceptGamble(state, teamId) {
  ...
  if (winner) {
    ...
    state.teams[winner].score += GAMBLE_STAKES;
    state.teams[loser].score -= GAMBLE_STAKES;
    pushLog(
      state,
      `Gamble: ${teamA.name} rolled ${rollA}, ${teamB.name} rolled ${rollB} — ${state.teams[winner].name} wins ${GAMBLE_STAKES} points!`
    );
  } else {
  ...
  logEvent(state, "gamble_resolve", { detail: `A:${rollA} B:${rollB} winner:${winner ?? "tie"}` });
  return state;
}
